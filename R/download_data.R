...
download_data <- function(trip_type, from, to, date){
  resp.post <- if (trip_type == 1) {
    CreateSession(origin = from, destination = to, startDate = date[1])
  } else {
    CreateSession(origin = from, destination = to,
  ...
  Out <- data$legs %>% filter(.data$Directionality == "Outbound") %>% 
    ...
    magrittr::set_names(c("OutboundLegId", "Out_DepartureTime", "Out_ArrivalTime",
                          "Out_Duration", "Out_No.Stops", "Out_Stops"))
  In <- data$legs %>% filter(.data$Directionality == "Inbound") %>% 
    select("Id", "DepartureTime", "ArrivalTime", "Duration", "No.Stops", "Stops") %>% 
    magrittr::set_names(c("InboundLegId", "In_DepartureTime", "In_ArrivalTime",
                          "In_Duration", "In_No.Stops", "In_Stops"))
  
  seg_carrier <- left_join(select(data$segments, .data$Id, .data$CarrierId),
                           select(data$carriers, .data$Id, .data$Code, .data$Name) %>% 
                             magrittr::set_names(c("CarrierId", "Code", "Name")), by = "CarrierId")
  
  segment_info <- data$legs %>%
    select(.data$Id, .data$SegmentIds) %>% 
    tidyr::unnest(.data$SegmentIds) %>% 
    ...
    tidyr::nest(-.data$LegId) %>%
    magrittr::set_names(c("LegId", "CarrierInfo"))
  
  flight <- data$itineraries %>% 
    left_join(Out, by = "OutboundLegId") %>% 
    left_join(In, by = "InboundLegId") %>% 
    left_join(select(data$price, -.data$SearchTime),
              by = c("OutboundLegId", "InboundLegId")) %>% 
    left_join(segment_info, by = c("OutboundLegId" = "LegId", "InboundLegId" = "LegId"))
  
  res <- flight %>%
    tidyr::unnest(.data$PricingOptions, .drop = FALSE) %>% 
    tidyr::unnest(.data$CarrierInfo, .drop = FALSE)
